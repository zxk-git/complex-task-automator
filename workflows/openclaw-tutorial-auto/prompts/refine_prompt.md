...
检测并修复代码块关闭标记错误（如 ` ```text` 紧跟在另一个代码块后）：
...
- 当 ``` 后紧跟标记（如 `text`、`bash`）且前面已有未关闭的代码块时，先插入 ``` 关闭前一个代码块
- 优先修复，因为后续所有操作都依赖正确的代码块边界

### 0b. 清洗爬虫/搜索残留（P1 自动修复）

移除由 Web 搜索或爬虫引入的原始残留内容：
- 删除 `### 补充 N`、`### 搜索结果` 等非正式小节
- 删除包含 `relevance:` 或 `source:` 等元数据标签的行
- 删除重复的空白行 (>3 连续空行 → 2 空行)
...
### 0d. GitHub Alert 语法统一（P1 自动修复）

将旧格式 Alert 转换为新版 GitHub Markdown Alert 语法：
...
支持类型: NOTE(💡/📝), TIP(💡), WARNING(⚠️), CAUTION(⛔), IMPORTANT(❗)
