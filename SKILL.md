...
version: 5.5.0
description: "高级任务自动化引擎 — 双模式流水线 (教程16阶段 + 代码5阶段)、LLM 扩写分析、多语言 i18n、HTML 可视化报告、目录发现、README自动更新、AI 精炼、插件热加载、YAML 评分规则引擎、交互式 CLI、异步任务队列、ECharts Dashboard。支持 8 语言代码分析、六维度质量检测、24/7 自动调度、多渠道通知 (飞书/企微/钉钉/Slack)。"
author: openclaw
metadata:
  tags:
    - automation
...
| 特性 | 说明 |
|------|------|
| **🔧 双模式流水线** | 教程优化 (16 阶段, 4 阶段并行) + 代码质量分析 (5 阶段)，BasePipeline 统一基类 |
| **🤖 AI 精炼** | OpenClaw Agent 驱动的智能优化（教程/代码/建议三模式） |
| **📝 LLM 扩写分析** | 自动识别短段落/缺失维度，生成结构化扩写 Prompt |
| **🌐 多语言 i18n** | zh-CN / en 双语消息目录 + Locale 切换 + 插件扩展 |
| **📊 HTML 可视化报告** | 静态 HTML + ECharts 5：得分柱状/饼图/雷达图/字数分布 |
| **🔌 插件热加载** | 28 种 Hook + 文件级插件自动发现 + 运行时重载 |
...
| **🔄 增量检测** | mtime+size 缓存 + 智能精炼阈值 + 锚点模糊匹配自动修复 |
| **🌐 多语言** | `--language en/zh-CN` 切换报告与日志语言 |
